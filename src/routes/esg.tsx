...
function ESGPage() {
  ...
  const reset = () => {
    setDate(""); setWater(""); setChrome(""); setSulphide(""); setSolidWaste(""); setCod("");
  };
      ...
      <div className="border border-border bg-card overflow-x-auto">
        <table className="w-full text-sm tabular">
          ...
          <tbody>
            {esgEntries.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-xs text-muted-foreground">No ESG records yet.</td>
              </tr>
            ) : (
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={submitESG} className="grid grid-cols-2 gap-3 pt-2">
            <div className="col-span-2 space-y-1.5">
              <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Date label</Label>
              <Input value={date} onChange={(e) => setDate(e.target.value)} placeholder="May 09" maxLength={20} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Water (m³)</Label>
              <Input type="number" min={0} value={water} onChange={(e) => setWater(e.target.value)} placeholder="150" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Chrome (kg)</Label>
              <Input type="number" min={0} value={chrome} onChange={(e) => setChrome(e.target.value)} placeholder="40" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Sulphide (kg)</Label>
              <Input type="number" min={0} value={sulphide} onChange={(e) => setSulphide(e.target.value)} placeholder="22" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Solid waste (kg)</Label>
              <Input type="number" min={0} value={solidWaste} onChange={(e) => setSolidWaste(e.target.value)} placeholder="430" />
            </div>
            <div className="col-span-2 space-y-1.5">
              <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Effluent COD (mg/L)</Label>
              <Input type="number" min={0} value={cod} onChange={(e) => setCod(e.target.value)} placeholder="2400" />
            </div>
            <DialogFooter className="col-span-2 mt-2">
              ...
              <button type="submit" className="bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:opacity-90">Log data</button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
